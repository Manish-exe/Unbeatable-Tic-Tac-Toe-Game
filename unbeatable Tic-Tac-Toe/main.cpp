...
// The 'pairs' array -> to check win condition
int pairs[8][3] = {{0, 3, 6}, {1, 4, 7}, {2, 5, 8}, {0, 1, 2}, {3, 4, 5}, {6, 7, 8}, {0, 4, 8}, {2, 4, 6}};

//'Corners' array -> used by AI to choose unoccupied corner
...
void cornerChoice(bool alreadyMoved)
{
    ...
    if (!alreadyMoved)
    {
        ...
        int temp = rand();
        board[goodChoices[temp % goodChoices.size()]] = 'X';
...
void aimove()
{
    ...
    int completes[8][3] = {{0, 3, 6}, {1, 4, 7}, {2, 5, 8}, {0, 1, 2}, {3, 4, 5}, {6, 7, 8}, {0, 4, 8}, {2, 4, 6}};
    ...
    else
    {
        ...
        for (int i = 0; i < 8; i++)
        {
            if (board[completes[i][0]] == 'X' && board[completes[i][1]] == 'X' &&
                board[completes[i][2]] != 'O')
            {
                board[completes[i][2]] = 'X';
                alreadyMoved = true;
            }
            else if (board[completes[i][1]] == 'X' &&
                     board[completes[i][2]] == 'X' &&
                     board[completes[i][0]] != 'O')
            {
                board[completes[i][0]] = 'X';
                alreadyMoved = true;
            }
            else if (board[completes[i][0]] == 'X' &&
                     board[completes[i][2]] == 'X' &&
                     board[completes[i][1]] != 'O')
            {
        ...
        for (int i = 0; i < 8; i++)
        {
            if (board[completes[i][0]] == 'O' && board[completes[i][1]] == 'O' &&
                board[completes[i][2]] != 'O')
            {
                board[completes[i][2]] = 'X';
                alreadyMoved = true;
            }
            else if (board[completes[i][1]] == 'O' &&
                     board[completes[i][2]] == 'O' &&
                     board[completes[i][0]] != 'O')
            {
                board[completes[i][0]] = 'X';
